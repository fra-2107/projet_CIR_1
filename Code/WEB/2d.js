...
function echelle(planete){
    ...
    for(element in planete){
        result[element] = [];
        result[element][0] = ((planete[element][0][0] * 150) / 9e+11);
        result[element][1] = -((planete[element][0][1] * 150) / 9e+11);
    };
...
function planete(data, i, couleur){
    ...
    {
        curveVertex(data[k][0]+windowWidth/2, data[k][1]+windowHeight/2, data[k][2]);
    }
    endShape();
    ...
    stroke(couleur)
    strokeWeight(7)
    point(data[i][0]+windowWidth/2, data[i][1]+windowHeight/2, data[i][2])
    strokeWeight(1);
}
...
function draw(){
    ...
    if(start)
    {
        ...
        stroke('red');
        strokeWeight(10);
        point(windowWidth/2,windowHeight/2,0);
        strokeWeight(1);
