...
Planet MethodEuler(Planet planet, int nbPoint, int deltaTemps){
    ...
    for (int i = 1; i < nbPoint; i+= deltaTemps){
        // printf("%d\n",i);   

        Point point = planet.trajectoire[i];

        // point.acceleration = calculAcceleration(planet, i-1);
        point.acceleration = scalaireVect(- (G * MASSE_SOLEIL) / pow(normeVect(planet.trajectoire[i-1].position),3), planet.trajectoire[i-1].position);

        point.position.x = planet.trajectoire[i-1].position.x + planet.trajectoire[i-1].vitesse.x * deltaTemps;
        point.position.y = planet.trajectoire[i-1].position.y + planet.trajectoire[i-1].vitesse.y * deltaTemps;
        // point.position.z = 0;
        point.vitesse.x = planet.trajectoire[i-1].vitesse.x + point.acceleration.x * deltaTemps;
        point.vitesse.y = planet.trajectoire[i-1].vitesse.y + point.acceleration.y * deltaTemps;
        // point.vitesse.z = 0;
